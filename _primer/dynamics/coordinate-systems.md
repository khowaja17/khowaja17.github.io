...
Note: $\mathbf{r}$ is explicitly not expressed in any reference frame and the vector equation elements $\mathbf{r},\hat{\mathbf n}_3$ are simply entities with a particular magnitude and direction in 3D space. As a result, $\mathbf{r} = x \hat{\mathbf{n}}_1 + a \hat{\mathbf{e}}_{\theta}$ is a valid vector equation. However, once we assign a reference frame to $\mathbf{r}$ like $\mathbf{r}^N$ then $\mathbf{r}^N \neq x \hat{\mathbf{n}}_1 + a \hat{\mathbf{e}}_{\theta}$. More generally $\mathbf{q} = \mathbf{r} + \mathbf{p}$, but $\mathbf{q}^N = \mathbf{r}^N + \mathbf{p}^E$ is not.   
